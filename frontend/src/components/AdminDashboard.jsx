...
export default function AdminDashboard({ onNavigate, theme = localStorage.getItem('theme') || 'dark' }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 📊 Dedicated Section: Defaulters Grouped by Created Task */}
      <div className={`rounded-2xl p-6 border transition-all ${
        ...
        <div className={`flex items-center justify-between mb-4 pb-3 border-b ${
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className={`text-base font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>
              Defaulters Grouped by Task ({defaultersData.grouped_defaulters?.length || 0} Active Tasks with Defaulters)
            </h3>
          </div>
          <button
        ...
        {(!defaultersData.grouped_defaulters || defaultersData.grouped_defaulters.length === 0) ? (
          <div className="py-8 text-center text-emerald-600 font-bold text-sm">
            🎉 All batch members are up to date! Zero defaulters across all tasks.
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {defaultersData.grouped_defaulters
              .filter(g => !selectedTaskId || String(g.task_id) === String(selectedTaskId))
              .map((group) => (
                <div 
                  key={group.task_id}
                ...
                >
                  <div className="flex items-center justify-between border-b pb-2 border-slate-200 dark:border-slate-700">
                    <div>
                      <h4 className={`text-sm font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>{group.task_title}</h4>
                      <span className="text-[10px] text-slate-500">Deadline: {group.due_date} at {group.due_time}</span>
                    </div>
                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-500/20 text-rose-600 border border-rose-500/30">
                      {group.total_defaulters} Defaulters
                    </span>
                  </div>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {group.defaulters.map((u) => (
                      ...
                      >
                        <div>
                          <p className={`font-bold ${isLight ? 'text-slate-900' : 'text-white'}`}>{u.user_name}</p>
                          <p className="text-[10px] text-slate-400">{u.user_email}</p>
                        </div>
                        <span className="text-[11px] font-bold text-rose-500">{u.days_late}</span>
                      </div>
                    ))}
                  </div>
                </div>
